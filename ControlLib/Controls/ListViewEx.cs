...
// Extended by Azure using example from http://support.microsoft.com/kb/822483/en-us to scroll when reordering,
// and some personal code for keeping gaps between selected and dragged items.
        ...
        protected override void OnDragDrop(DragEventArgs drgevent)
        {
            if (this.IsRowDragInProgress)
            {
                try
                {
                    ...
                    if (dropItem != null)
                    {
                        ListViewItem[] dragItems = new ListViewItem[this.SelectedItems.Count];
                        for (int i = 0; i <= this.SelectedItems.Count - 1; i++)
                        {
                            dragItems[i] = this.SelectedItems[i];
                        }
                        ...
                        for (int i = 0; i < dragItems.Length; ++i)
                        {
                            ...
                            if (i > 0)
                            {
                                indexDiff = (dragItem.Index - prevIndex);
                            }
                            ...
                            {
                                dropIndex += (1 + indexDiff);
                            }

                            prevIndex = dragItem.Index;
        ...
        protected override void OnDragOver(DragEventArgs drgevent)
        {
            if (this.IsRowDragInProgress)
            {
                ...
                if (dropItem != null)
                {
                    ...
                    insertionMode = InsertionMode.After;//clientPoint.Y < bounds.Top + (bounds.Height / 2) ? InsertionMode.Before : InsertionMode.After;
