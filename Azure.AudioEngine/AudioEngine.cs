namespace Azure.MediaUtils
{
	...
	public sealed class AudioEngine : IDisposable
	{
		...
		public AudioEngine(Int32 deviceID, String modulePath, IntPtr handle, bool usePlugins = false, String pluginPath = null)
		{
			Utils.Prepare();
			this.Hwnd = handle;
			bool libLoaded = Bass.LoadMe(modulePath);
			...
			try
			{
				if (this.CurrentState != PlaybackState.Paused)
				{
					...
					do
					{
						...
						{
							this.ChangeChannelVolume(this.channelVolume);
							break;
						}
						else
						{
		...
		public void ChangeChannelVolume(float value)
		{
		...
		public string ChannelGetInfo(string param)
		{
			BASS_CHANNELINFO channelInfo = Bass.BASS_ChannelGetInfo(this.stream);
			switch (param)
			{
				case "chan": return channelInfo.chans.ToString();
				case "freq": return channelInfo.freq.ToString();
				case "chanStr": return BassUtils.ChannelNumberToString(channelInfo.chans);
